...
public class OldPlayMenuScreen extends MovingBackground implements Screen, MenuTextures {
    ...
    public OldPlayMenuScreen(Khartoosha game)
    {
    ...
    public void chosenTexture(int dynamicTextureNum) {
        if(dynamicTextureNum == 1) { //if one player is clicked
            this.dispose();
            game.setScreen(new OldCharacterChoiceMenuScreen(game, false));
        } else if (dynamicTextureNum == 2) { //if two players is clicked
            this.dispose();
            game.setScreen(new OldCharacterChoiceMenuScreen(game, true));
